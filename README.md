...
* [golden/near0/](./SIM/golden/near0/) — golden `.jls` reference files for NEAR=0 (lossless), generated by a verified software encoder.
...
The runner compiles the testbench, runs each image through `jls_encoder`, and compares every output byte against the golden file. Example output:
...
* [golden/near0/](./SIM/golden/near0/) — NEAR=0（无损）模式的黄金 `.jls` 参考文件。
